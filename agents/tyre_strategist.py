...
When called, queries lap history through MCP, builds a structured prompt, and
asks the LLM to assess the focal driver's current stint. Returns a
TyreAssessment.

The strategist is one of seven specialists. It does not make pit
recommendations — that's the Orchestrator's job. Its only output is
"where's the cliff and how confident are you."
"""
import os
...
SYSTEM_PROMPT = """You are the Tyre Strategist on an F1 team's pit wall.
...
def _build_user_prompt(driver_code, current_lap, history, stints) -> str:
    ...
    Kept compact: every token costs. Left-aligned so leading whitespace
    inside the multi-line f-string doesn't bloat the request.
    """
    ...
    # Recent lap times, most recent last.
    recent = stint_laps[-8:]
    lap_lines = "\n".join(
...
async def assess_tyres(
    ...
    detailed lap history and stint structure, then asks the LLM to assess.
    ...
    if state is None:
        raise RuntimeError("No live RaceState active — cannot assess tyres.")
